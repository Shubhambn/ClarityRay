...
class ModelDef:
    ...
    # Per-label control (multilabel view only).
    # When restrict_suspicious=True, only labels in suspicious_findings get
    # suspicious: True; all others are reported at suppressed_threshold.
    restrict_suspicious: bool = False
    per_label_thresholds: dict = field(default_factory=dict)
    suppressed_threshold: float = 0.70
    # Explainability block — written to clarity.json when grid_size is set.
    grid_size: int | None = None
    explainability_disclaimer: str | None = None
...
MODEL_REGISTRY: dict[str, ModelDef] = {
    ...
    "densenet121-lung-cancer-nih14": ModelDef(
        key="densenet121-lung-cancer-nih14",
        slug="densenet121-lung-cancer-nih14",
        name="NIH14 Lung Cancer Detector (DenseNet-121)",
        loader=_load_torchxrayvision("densenet121-res224-nih"),
        suspicious_findings=["Mass", "Nodule", "Pleural_Thickening"],
        restrict_suspicious=True,
        per_label_thresholds={
            "Mass": 0.28,
            "Nodule": 0.22,
            "Pleural_Thickening": 0.38,
        },
        suppressed_threshold=0.70,
        channels=1,
        input_size=224,
        grid_size=32,
        family="DenseNet121",
        source_description="TorchXRayVision densenet121-res224-nih (NIH ChestX-ray14 — 112,120 images, 14 pathologies)",
        disclaimer=(
            "Lung cancer screening aid using DenseNet-121 trained on NIH ChestX-ray14 "
            "(112,120 frontal-view X-rays). Configured exclusively to detect pulmonary "
            "malignancy indicators: Mass (threshold 0.28), Nodule (threshold 0.22), and "
            "Pleural Thickening (threshold 0.38). Non-cancer findings are suppressed at "
            "elevated thresholds to minimise noise. This is NOT a diagnostic tool. All "
            "results require immediate review by a qualified radiologist or thoracic "
            "oncologist. Never make clinical decisions based on this output alone."
        ),
        explainability_disclaimer=(
            "Heatmap generated by high-resolution occlusion sensitivity (32×32 grid, "
            "1,024 inference passes per image). Each grid cell was masked with zero-intensity "
            "and the model re-ran to measure that region's contribution to the lung cancer score. "
            "Warmer regions (red/orange) drove the Mass or Nodule prediction most strongly — "
            "cooler regions (blue) had minimal impact. The 32×32 resolution delivers 256% "
            "more spatial detail than standard 20×20 configurations, enabling precise "
            "lesion localisation. This is an approximation of correlation, not causation, "
...
def _build_passthrough_wrapper(base: Any, pathologies: list[str]):
    ...
    TorchXRayVision uses a shared N-slot output space across all checkpoints.
    Some models (e.g. densenet121-res224-nih) only fill 14 of 18 slots; the
    remaining slots are empty strings. We filter them here so the exported
    ONNX graph only has columns for real pathologies.
    """
    import torch  # type: ignore[import-not-found]

    valid_indices = [i for i, p in enumerate(pathologies) if p.strip()]
    filtered = [pathologies[i] for i in valid_indices]
    needs_filter = len(valid_indices) < len(pathologies)

    if needs_filter:
        class SelectivePassthrough(torch.nn.Module):
            ...
            def forward(self, x: "torch.Tensor") -> "torch.Tensor":
                return self.model(x).index_select(1, self.indices)

        wrapper = SelectivePassthrough(base, valid_indices).eval()
    else:
        class PassthroughWrapper(torch.nn.Module):  # type: ignore[no-redef]
...
def _build_spec(
    ...
    if view == "multilabel":
        ...
        # threshold. When restrict_suspicious=True only the curated cancer subset
        # is flagged as suspicious; all other labels are suppressed at a high
        # threshold so they don't create noise in the clinical display.
        name = model_def.name
        suspicious_set = set(model_def.suspicious_findings) if model_def.restrict_suspicious else None
        output = {
            "task": "multilabel",
            ...
            "labels": [
                {
                    "name": p,
                    "threshold": model_def.per_label_thresholds.get(
                        p,
                        DEFAULT_LABEL_THRESHOLD if (suspicious_set is None or p in suspicious_set)
                        else model_def.suppressed_threshold,
                    ),
                    "suspicious": suspicious_set is None or p in suspicious_set,
                }
    ...
    return {
        ...
        **({"explainability": {
            ...
            "isModelBased": True,
            "isGradCAM": False,
            "grid_size": model_def.grid_size,
            "disclaimer": model_def.explainability_disclaimer or (
                f"Heatmap generated by occlusion sensitivity ({model_def.grid_size}×"
                f"{model_def.grid_size} grid, {model_def.grid_size ** 2} inference passes per image). "
                "Warmer regions contributed most to the detected finding. Not for clinical use."
            ),
        }} if model_def.grid_size else {}),
    }
...
def export_one(model_def: ModelDef, view: str = "multilabel") -> Path:
    ...
    classes = pathologies  # default; overwritten below for multilabel view
    if view == "multilabel":
    ...
    spec = _build_spec(model_def, sha, view, classes, matched)
