...
export function ContrastViewer({ imageUrl, heatmap, embedded = false, className }: ContrastViewerProps) {
    ...
    <div className="relative flex min-h-[200px] w-full flex-1 items-center justify-center overflow-hidden rounded-lg bg-slate-950/80">
      {hasImage ? (
        <canvas
          ...
          aria-label="X-ray scan with synthetic contrast overlay"
        />
      ) : (
        <div className="flex h-full min-h-[200px] w-full items-center justify-center px-4 text-center text-sm text-slate-500">
          Upload and run analysis to render the scan and contrast overlay.
        </div>
      )}
    </div>
  );
  ...
  const footnote = (
    <p className="mt-2 text-xs italic text-zinc-500">
      This overlay is derived from image contrast, not model gradients. It is a visual aid only and
      does not represent model-derived spatial explanations. Not a diagnostic feature.
    </p>
  );
  ...
  return (
    <div className={cn('card space-y-3', className)}>
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-white">Visual contrast overlay (synthetic)</h3>
        <span className="text-xs text-slate-400">Contrast-based overlay</span>
      </div>
      {canvasBlock}
      {footnote}
    </div>
